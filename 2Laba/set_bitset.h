...
class Set {
private:
    unsigned int bits;  // битовое представление множества
    char tag;           // тег для идентификации
...
public:
    ...
    Set() : bits(0), tag('A' + object_count++) {
        printDebug("Конструктор по умолчанию");
    }
    
    Set(const char* str) : bits(0), tag('A' + object_count++) {
    ...
    Set(const Set& other) : bits(other.bits), tag('A' + object_count++) {
        printDebug("Конструктор копирования");
    }
    
    Set(Set&& other) noexcept : bits(other.bits), tag('A' + object_count++) {
        other.bits = 0;
    ...
    Set operator~() const {
        ...
        Set result;
        result.bits = ~bits & ((1u << UNIVERSAL_SIZE) - 1);
        return result;
    }
    ...
    static Set universal() {
        Set result;
        result.bits = (1u << UNIVERSAL_SIZE) - 1;
        return result;
    }
